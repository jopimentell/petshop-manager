{{-- resources/views/admin/agendamentos/edit.blade.php --}}
@extends('adminlte::page')

@section('title', 'Editar Agendamento - PetShop')
            ...
            <form action="{{ route('admin.agendamentos.update', $agendamento) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select name="animal_id" id="animal_id" 
                                ...
                                <option value="{{ $animal->id }}" 
                                        {{ old('animal_id', $agendamento->animal_id) == $animal->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4">
                            <div class="form-check">
                                <input type="checkbox" name="servicos[]" value="{{ $servico->id }}" 
                                       ...
                                       {{ in_array($servico->id, old('servicos', $agendamento->servicos->pluck('id')->toArray())) ? 'checked' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="situacao" id="situacao" 
                                ...
                                <option value="agendado" {{ old('situacao', $agendamento->situacao) == 'agendado' ? 'selected' : '' }}>Agendado</option>
                                <option value="confirmado" {{ old('situacao', $agendamento->situacao) == 'confirmado' ? 'selected' : '' }}>Confirmado</option>
                                <option value="em_andamento" {{ old('situacao', $agendamento->situacao) == 'em_andamento' ? 'selected' : '' }}>Em Andamento</option>
                                <option value="concluido" {{ old('situacao', $agendamento->situacao) == 'concluido' ? 'selected' : '' }}>Concluído</option>
                                <option value="cancelado" {{ old('situacao', $agendamento->situacao) == 'cancelado' ? 'selected' : '' }}>Cancelado</option>
                                <option value="nao_compareceu" {{ old('situacao', $agendamento->situacao) == 'nao_compareceu' ? 'selected' : '' }}>Não Compareceu</option>
                            </select>
                            @error('situacao')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="valor_total" id="valor_total" step="0.01" min="0"
                                       ...
                                       value="{{ old('valor_total', $agendamento->valor_total) }}" readonly>
                                @error('valor_total')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="observacoes" id="observacoes" 
                              ...
                              rows="3">{{ old('observacoes', $agendamento->observacoes) }}</textarea>
                    @error('observacoes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <a href="{{ route('admin.agendamentos.show', $agendamento) }}" class="btn btn-default">
                        Cancelar
                    </a>
                </div>
            </form>
